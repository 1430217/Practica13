...
class Datos extends Conexion{
    ...
    public function addJugadorModel($jugador, $tabla){
        $stmt = Conexion::conectar()->prepare("INSERT INTO $tabla (nombre_jugador, numero_jugador, idEquipo) 
                VALUES (:nombre_jugador, :numero_jugador, :idEquipo)"
        );
        ...
        $stmt->bindParam(':idEquipo', $jugador['idEquipo'], PDO::PARAM_INT);
        if($stmt->execute()) 
            return 'success';
        else  
            return 'error'; 
    ...
    public function updateJugadorModel($jugador, $tabla){
        $stmt = Conexion::conectar()->prepare("UPDATE $tabla SET nombre_jugador = :nombre_jugador, numero_jugador = :numero_jugador,
            idEquipo = :idEquipo WHERE idJugador = :idJugador")
        ;

        $stmt->bindParam(':nombre_jugador', $jugador['nombre_jugador'], PDO::PARAM_STR);
        $stmt->bindParam(':numero_jugador', $jugador['numero_jugador'], PDO::PARAM_STR);
        $stmt->bindParam(':idEquipo', $jugador['idEquipo'], PDO::PARAM_INT);
        $stmt->bindParam(':idJugador', $jugador['idJugador'], PDO::PARAM_INT);
    ...
    public function getJugadores(){
        $stmt = Conexion::conectar()->prepare("CALL getJugadores()");
        $stmt->execute();
        return $stmt->fetchAll();
    ...
    //Funcion para traer solo un registro de la base de datos
    public static function getJugador($idJugador, $tabla){
        $stmt = Conexion::conectar()->prepare("SELECT * FROM $tabla WHERE idJugador = :idJugador");
    ...
    //Funcion para borrar los equipos
    public function deleteJugador($idJugador, $tabla){
		$stmt = Conexion::conectar()->prepare("DELETE FROM $tabla WHERE idJugador = :idJugador");	
        $stmt->bindParam(":idJugador", $idJugador, PDO::PARAM_INT);
